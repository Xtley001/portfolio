  ...
  return (
    <div
      style={{
        ...
        padding: small ? 'clamp(20px, 3vw, 28px)' : 'clamp(28px, 4vw, 40px)',
        minHeight: small ? '160px' : '220px',
        display: 'flex',
    ...
    >
      <div>
        ...
        <p style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '13px', color: 'var(--text-dim)', lineHeight: 1.8 }}>
          {tagline}
        </p>
      </div>
      ...
      <div>
        {tech.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '18px' }}>
            {tech.map(t => (
              <span key={t} style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '10px', letterSpacing: '0.1em', color: 'var(--text-faint)', border: '1px solid var(--border)', padding: '3px 10px', borderRadius: '2px' }}>
                {t}
              </span>
            ))}
          </div>
        ...
        <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
          ...
          <Link href={`/projects/${project.slug}`}
            style={{ marginLeft: 'auto', fontFamily: 'var(--font-dm-mono)', fontSize: '11px', color: 'var(--text-dim)', letterSpacing: '0.08em', transition: 'color 0.15s' }}
            onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent)')}
            onMouseLeave={e => (e.currentTarget.style.color = 'var(--text-dim)')}
          >
            details →

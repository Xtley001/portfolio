    ...
    <section ref={ref} id="contact" style={{ padding: 'clamp(40px, 8vw, 80px) clamp(16px, 4vw, 40px) 120px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div className="contact-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'start' }}>
        <div className="reveal">
          <h2 style={{ fontFamily: 'var(--font-syne)', fontWeight: 800, fontSize: 'clamp(2.4rem, 5vw, 4rem)', lineHeight: 1, letterSpacing: '-0.02em', color: 'var(--text)', marginBottom: '28px' }}>
            LET&apos;S<br />TALK.
          </h2>
          {contact.note && (
            <p style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '15px', color: 'var(--text-dim)', lineHeight: 1.9, maxWidth: '320px' }}>
              {contact.note}
            </p>
          )}
          {contact.email && (
            <a
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', marginTop: '32px', background: 'var(--accent)', color: 'var(--bg)', fontFamily: 'var(--font-dm-mono)', fontSize: '12px', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '14px 28px', borderRadius: '2px', transition: 'opacity 0.15s' }}
        ...
        <div className="reveal" style={{ display: 'flex', flexDirection: 'column' }}>
          {links.map(link => (
            <a
              ...
              style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '20px 0', borderBottom: '1px solid var(--border)', transition: 'all 0.2s', color: 'var(--text-dim)' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--accent)'; (e.currentTarget as HTMLElement).style.paddingLeft = '6px' }}
            ...
            >
              ...
              <span style={{ flex: 1, fontFamily: 'var(--font-dm-mono)', fontSize: '11px', color: 'var(--text-faint)', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
                {link.label}
              </span>
              <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: '13px', letterSpacing: '0.03em' }}>
                {link.display}
              </span>
              <ArrowIcon />
            </a>
